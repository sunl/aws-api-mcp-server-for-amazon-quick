...
@server.tool(
    ...
    - Pass ``target_account_id`` (a 12-digit AWS account ID) to run the command
      against a different AWS account via STS AssumeRole.
    - Leave ``target_account_id`` unset to run against the account this server
      is deployed in (the default).
    - The target account must contain a role matching the configured
      cross-account role name whose trust policy permits this server to assume it.
    - ``target_account_id`` applies to every command in a batch. If the user asks
      about multiple accounts in one request, make one ``call_aws`` invocation
      per account rather than mixing them in a single batch.
